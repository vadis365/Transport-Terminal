...
public class BlockCharger extends BlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (worldIn.isRemote)
			return true;
		if (worldIn.getTileEntity(pos) instanceof TileEntityCharger)
			player.openGui(TransportTerminal.instance, TransportTerminal.proxy.GUI_ID_CHARGER, worldIn, pos.getX(), pos.getY(), pos.getZ());
		return true;
	}
