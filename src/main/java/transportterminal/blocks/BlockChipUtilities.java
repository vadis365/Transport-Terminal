...
public class BlockChipUtilities extends BlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (worldIn.isRemote)
			return true;
		if (worldIn.getTileEntity(pos) instanceof TileEntityChipUtilities)
			player.openGui(TransportTerminal.instance, TransportTerminal.proxy.GUI_ID_CHIP_UTILS, worldIn, pos.getX(), pos.getY(), pos.getZ());
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if (tileentity instanceof TileEntityChipUtilities)
			InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityChipUtilities) tileentity);
