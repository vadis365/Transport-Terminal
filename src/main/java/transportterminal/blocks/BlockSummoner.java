...
public class BlockSummoner extends BlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (worldIn.isRemote)
			return true;
		if (worldIn.getTileEntity(pos) instanceof TileEntitySummoner)
			player.openGui(TransportTerminal.instance, TransportTerminal.proxy.GUI_ID_SUMMONER, worldIn, pos.getX(), pos.getY(), pos.getZ());
		return true;
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if (tileentity instanceof TileEntitySummoner)
			InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntitySummoner) tileentity);
		super.breakBlock(worldIn, pos, state);
	}

	@Override
	public int getRenderType() {
		return 3;// TODO Fixy BlockRenderIDs.SUMMON_BLOCK.id();
	}
