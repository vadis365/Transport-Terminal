...
public class PlayerSummonPacketHandler implements IMessageHandler<PlayerSummonMessage, IMessage> {
	...
	public IMessage onMessage(PlayerSummonMessage message, MessageContext ctx) {
		...
		EntityPlayerMP playerOnChip = MinecraftServer.getServer().getConfigurationManager().createPlayerForUser(new GameProfile(UUID.nameUUIDFromBytes(message.playerOnChip.getBytes()), message.playerOnChip));
